(function initCatalogDesignModule(root, factory) {
    ...
    function getConceptName(product, variant) {
        ...
        if (/^v\d+$/i.test(name)) {
            name = `${stripBandPrefix(productBaseName, product?.band)} ${name}`.trim();
        }
    ...
    function buildCatalogDesigns(products, options = {}) {
        ...
        function ensureGroup(product, conceptName, explicitId) {
            const band = getPublicBand(product);
            const transitionId = `${slugify(band)}-${slugify(conceptName)}`;
            const designId = explicitId || transitionId;
            if (!groups.has(designId)) {
                groups.set(designId, {
                    ...
                    sourceProductIds: [],
                    designFamilyIds: [],
                    identitySource: explicitId ? 'explicit-map' : 'transition-name'
                });
            }
            ...
            variants.forEach((variant, variantIndex) => {
                ...
                if (source.role === 'back') {
                    ...
                    if (!targetDesignId || !groups.has(targetDesignId)) {
                        const orphanGroup = ensureGroup(product, conceptName, explicitId);
                ...
                const group = ensureGroup(product, conceptName, explicitId);
